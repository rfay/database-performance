...
* [build-and-push-seeded-image.sh](scripts/build-and-push-seeded-image.sh) - Builds (and optionally `--push`es) a multi-platform (linux/amd64+arm64) standalone dbimage with a base_db seed baked in, for the `dbimage:` technique above. Modeled on ddev/ddev's own `containers/ddev-dbserver/build_image.sh`.
